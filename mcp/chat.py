...
def _llm_reply(
    ...
    for _ in range(MAX_TOOL_ITERATIONS):
        ...
        # Серверний веб-пошук і наші локальні інструменти в ОДНІЙ відповіді —
        # несумісні (прод 2026-08-11): модель повертає і tool_use, і
        # server_tool_use, ми дописуємо tool_result лише для свого, і
        # наступний виклик падає з 400 "container_id is required when there
        # are pending tool uses generated by code execution". Сервер
        # добудовує свою частину сам, якщо його не переривати — тож коли в
        # ході є server_tool_use, ми НЕ втручаємось: віддаємо цю ітерацію
        # серверу (порожній user-хід), і він завершує пошук у наступному
        # раунді. Локальні виклики з того ж ходу модель повторить сама.
